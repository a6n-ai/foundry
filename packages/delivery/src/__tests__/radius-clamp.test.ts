...
import { clampRadiusKm, type Zone } from "../zones";

const zone = (publicId: string, radiusKm: number): Zone => ({
  publicId,
  name: publicId,
  radiusKm,
  postalPrefixes: [],
...
describe("clampRadiusKm (ported from puchkaman)", () => {
  ...
  it("ignores postal zones", () => {
    expect(clampRadiusKm(7, [...ZONES.slice(1), { ...zone("zon_postal", 0), radiusKm: null, postalPrefixes: ["M9"] }], "zon_inner")).toBe(7);
  });
