...
import { destinationPoint, haversineKm } from "../distance";
import { clampRadiusKm } from "../zones";
...
/** Ring clamp for live drag feedback — the package's clampRadiusKm, fed map rings. The server re-clamps. */
export function clampMapRadius(radiusKm: number, zones: MapZone[], excludePublicId: string): number {
  return clampRadiusKm(
    radiusKm,
    zones.map((z) => ({ ...z, postalPrefixes: [], slotWindow: null })),
    excludePublicId,
  );
}
        ...
        if (existing) {
          ...
          marker.on("dragend", () => {
            ...
            latest.current.onRadiusCommit(
              publicId,
              clampMapRadius(haversineKm(o.lat, o.lng, lat, lng), latest.current.zones, publicId),
            );
          });
