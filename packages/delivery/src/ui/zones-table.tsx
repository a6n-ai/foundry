...
import type { TypeOption, ZoneRow } from "./context";
...
const COLUMNS_NO_TYPES = COLUMNS.filter((c) => c.key !== "offers");

const fmt = (n: number) => (Number.isInteger(n) ? String(n) : n.toFixed(2).replace(/0$/, ""));

/**
 * Postal zones first (by name), then circles by radius. A circle shows the band it
 * covers, not a bare radius — its inner edge is implied by the ring inside it.
 */
export function ZonesTable({
  ...
  const liveRadius = (z: ZoneRow) =>
    mapZones.find((m) => m.publicId === z.publicId)?.radiusKm ?? z.radiusKm ?? 0;
  const isCircle = (z: ZoneRow) => z.radiusKm != null;

  const ordered = [...zones].sort((a, b) =>
    isCircle(a) !== isCircle(b) ? (isCircle(a) ? 1 : -1) : isCircle(a) ? liveRadius(a) - liveRadius(b) : a.name.localeCompare(b.name),
  );
  const activeRadii = ordered.filter((z) => z.active && isCircle(z)).map(liveRadius);
  ...
  return (
    <DataTable
      columns={hasTypes ? COLUMNS : COLUMNS_NO_TYPES}
      rows={ordered}
      rowKey={(z) => z.publicId}
      serial={false}
          ...
          <TableCell className="tabular-nums">
            {!isCircle(z) ? (
              <span className="flex flex-col">
                <span>{z.postalPrefixes.join(", ")}</span>
                {z.slotWindow && <span className="text-muted-foreground text-xs">{z.slotWindow}</span>}
              </span>
            ) : z.active ? (
              `${fmt(innerEdgeFor(z))}–${fmt(liveRadius(z))} km`
            ) : (
              "—"
            )}
          </TableCell>
