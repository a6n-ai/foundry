...
import type { DeliveryTypeInput } from "../service";
import { useDeliveryActions } from "./context";

type TypeFormValues = DeliveryTypeInput;
...
export function TypeEditDialog({
  ...
  function save() {
    ...
    start(async () => {
      const res = await actions.saveType({ ...form, minSubtotal, discountPct, sortOrder });
      if (res.error) {
  ...
  function retire() {
    ...
    startRetire(async () => {
      const res = await actions.retireType(form.publicId!);
      if (res.error) {
        toast.error(res.error);
        return;
      }
